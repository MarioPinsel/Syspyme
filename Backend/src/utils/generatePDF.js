...
function agregarNuevaPagina(doc) {
   doc.addPage({
      size: 'LETTER',
...
function dibujarHeaderTabla(doc, yPos, tableLeft, tableWidth, col1Width, col2Width, col3Width, col4Width, col5Width, primaryColor) {
...
export async function generarPDFBuffer(xmlString) {
   ...
   const ALTURA_PAGINA = 792;
   const ALTURA_FILA = 24;
   ...
   yPos = dibujarHeaderTabla(doc, yPos, tableLeft, tableWidth, col1Width, col2Width, col3Width, col4Width, col5Width, primaryColor);

   // Dibujar filas de productos con control de paginación
   productos.forEach((p, index) => {
      // Verificar si necesitamos una nueva página
      if (yPos + ALTURA_FILA > ALTURA_PAGINA - MARGEN_INFERIOR) {
         yPos = agregarNuevaPagina(doc);

         // Dibujar título y header de tabla en la nueva página
         doc.fontSize(14)
            .fillColor(primaryColor)
            .font('Helvetica-Bold')
            .text('Detalle de productos (continuación)', 40, yPos);
      ...
      doc.rect(tableLeft, yPos, tableWidth, ALTURA_FILA)
         .fillAndStroke(bgColor, borderColor)
         .lineWidth(1);
   ...
   // ==================== TOTALES ====================
   // Verificar espacio para totales (necesitamos ~200px)
   if (yPos + 200 > ALTURA_PAGINA - MARGEN_INFERIOR) {
      yPos = agregarNuevaPagina(doc);
   }
   ...
   // Verificar espacio para CUFE + Footer
   if (yPos + cufeHeight + 80 > ALTURA_PAGINA - MARGEN_INFERIOR) {
      yPos = agregarNuevaPagina(doc);
   }
